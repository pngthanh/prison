...
$stmt = $conn->prepare("
    SELECT 
        p.*, 
        c.Ten_buong, 
        c.Trang_thai AS Trang_thai_buong, 
        c.Suc_chua
    FROM prisoners p
...
<head>
    ...
    <link rel="stylesheet" href="css/prisoners_detail.css">
</head>
            ...
            <div class="prisoners-info">
                ...
                <div class="prisoners-details">
                    ...
                    <p><strong>Buồng giam:</strong>
                        <?php echo htmlspecialchars($prisoner['Ten_buong'] ?? 'Chưa được phân công'); ?></p>
                    <p><strong>Trạng thái:</strong> <?php echo $trang_thai; ?></p>
                    <p><strong>Số CMND:</strong> <?php echo $so_CMND; ?></p>
                    <p><strong>Điểm hoạt động:</strong> <?= htmlspecialchars($prisoner['Tong_diem_hoat_dong'] ?? 0) ?></p>
                </div>
            </div>
            <!-- Nút -->
            <div class="actions">
                <?php
                $back_url = isset($_GET['cell_id']) && !empty($_GET['cell_id']) ? "cells_detail.php?id=" . intval($_GET['cell_id']) : "prisoners.php";
                ?>
